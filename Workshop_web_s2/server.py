...
# numServ = random.randint(0,500)


# EXERCICE PLUS OU MOINS 
# @app.route("/formulaire", methods=['GET','POST'])
# def PlusOuMoins() :
#         variable = "Super Jeu de Ouf !"
#         if request.method == 'POST' :
#             numClient = request.form['number']
            
#             if int(numClient) < numServ :
#                 variable="Vous etes en dessous"
#             elif int(numClient) > numServ :
#                 variable="Vous etes au dessus"
#             elif int(numClient) == numServ :
#                 variable="IT iS GOOOOOD"
            
#         return render_template('serveurplusoumoins.html', variable=variable)



# EXO FORM JEU
tabgames = [{'name':'SuperMario' , 'price':'60' , 'description':'champignon'}]

@app.route("/")
def index():
    return render_template('liste.html', games=tabgames)
...
def ajout():
    if request.method == 'POST' :
        ...
        add_game = {'name':add_name, 'price':add_price, 'description':add_description}
        tabgames.append(add_game)
        return render_template('liste.html', games=tabgames)